...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const projectData = {
        '1': {
            ...
            details: 'Desarrollada para ofrecer una solución robusta y centralizada, esta aplicación optimiza los procesos administrativos del club, reduciendo el trabajo manual y mejorando la precisión de los datos. La <span class="easter-egg-word" data-egg-text="There is no spoon.">gestión</span> de la interfaz, construida con JavaFX, fue diseñada para ser intuitiva y fácil de usar para personal no técnico.',
            tech: ['Java', 'JavaFX', 'MySQL']
        },
        '2': {
            title: 'Inova Nexus',
            type: 'App Móvil Nativa para Android',
            details: 'Inova Nexus es una herramienta integral pensada por y para amantes de la literatura. Permite a los usuarios organizar sus lecturas, hacer seguimiento de su progreso de escritura y tomar notas. La arquitectura nativa en Kotlin asegura un rendimiento fluido y una experiencia de usuario óptima.',
            tech: ['Kotlin', 'Android Nativo', 'SQLite']
        },
        '3': {
            title: 'Calcutron',
            type: 'Herramienta de Cómputo para Escritorio',
            details: 'Inspirada en la estética retro-futurista de "Tron", esta aplicación combina funcionalidad con un diseño único, proveyendo soluciones visuales para problemas de estadística, operaciones con matrices y cálculo de integrales.',
            tech: ['Java', 'JavaFX']
        }
    };
    ...
    closeProyectoModalBtn.addEventListener('click', () => proyectoModal.classList.remove('active'));
...
// Lógica para el link "AlgoDev" en la nav (Sin cambios)
...
// Inyectar botones de contacto del equipo (Sin cambios)
...
teamContactContainer.innerHTML = contactHTML;
    ...
    rabbitBtn.addEventListener('click', () => {
        ...
        const rabbitTyped = new Typed(typedTarget, {
            strings: ['> Sigue al conejo blanco...'],
            typeSpeed: 50,
            loop: false,
            showCursor: true,
            cursorChar: '|',
    ...
    // --- 4.2 "Un Fallo en la Matrix" (Tech Stack) --- (Sin cambios)
    const techItems = document.querySelectorAll('.tech-item');
    if (techItems.length > 0) {
        const randomIndex = Math.floor(Math.random() * techItems.length);
        const glitchItem = techItems[randomIndex];
        glitchItem.classList.add('glitch');
        glitchItem.setAttribute('data-text', glitchItem.dataset.techName);
        
        glitchItem.addEventListener('mouseenter', () => {
            glitchItem.classList.remove('glitch');
        }, { once: true });
    }

    // --- 4.3 "There is no spoon" (Función para la Modal) --- (Sin cambios)
    function triggerNoSpoonEasterEgg() {
        const eggText = this.dataset.eggText;
        proyectoModal.classList.remove('active');
